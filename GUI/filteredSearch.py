# -*- coding: utf-8 -*-

# Form implementation generated from reading ui file 'filteredSearch.ui'
#
# Created by: PyQt5 UI code generator 5.15.4
#
# WARNING: Any manual changes made to this file will be lost when pyuic5 is
# run again.  Do not edit this file unless you know what you are doing.


from PyQt5 import QtCore, QtGui, QtWidgets


class Ui_filteredResults(object):
    def setupUi(self, filteredResults):
        filteredResults.setObjectName("filteredResults")
        filteredResults.resize(800, 800)
        filteredResults.setStyleSheet("background-color: rgb(54, 54, 54);")
        self.tableFilteredResults = QtWidgets.QTableWidget(filteredResults)
        self.tableFilteredResults.setGeometry(QtCore.QRect(30, 140, 741, 631))
        self.tableFilteredResults.setStyleSheet("color: rgb(255, 255, 255);")
        self.tableFilteredResults.setObjectName("tableFilteredResults")
        self.tableFilteredResults.setColumnCount(0)
        self.tableFilteredResults.setRowCount(0)
        self.labelFiltered = QtWidgets.QLabel(filteredResults)
        self.labelFiltered.setGeometry(QtCore.QRect(140, 40, 201, 31))
        font = QtGui.QFont()
        font.setPointSize(12)
        self.labelFiltered.setFont(font)
        self.labelFiltered.setStyleSheet("color: rgb(255, 255, 255)")
        self.labelFiltered.setObjectName("labelFiltered")
        self.labelFiltered_2 = QtWidgets.QLabel(filteredResults)
        self.labelFiltered_2.setGeometry(QtCore.QRect(370, 40, 191, 31))
        font = QtGui.QFont()
        font.setPointSize(12)
        self.labelFiltered_2.setFont(font)
        self.labelFiltered_2.setStyleSheet("color: rgb(255, 255, 255)")
        self.labelFiltered_2.setObjectName("labelFiltered_2")
        self.psoitive = QtWidgets.QPushButton(filteredResults)
        self.psoitive.setGeometry(QtCore.QRect(150, 110, 101, 23))
        self.psoitive.setStyleSheet("color: rgb(255, 255, 255)")
        self.psoitive.setObjectName("psoitive")
        self.specials = QtWidgets.QPushButton(filteredResults)
        self.specials.setGeometry(QtCore.QRect(30, 110, 101, 23))
        self.specials.setStyleSheet("color: rgb(255, 255, 255)")
        self.specials.setObjectName("specials")
        self.negative = QtWidgets.QPushButton(filteredResults)
        self.negative.setGeometry(QtCore.QRect(270, 110, 101, 23))
        self.negative.setStyleSheet("color: rgb(255, 255, 255)")
        self.negative.setObjectName("negative")
        self.valveFilter = QtWidgets.QPushButton(filteredResults)
        self.valveFilter.setGeometry(QtCore.QRect(390, 110, 101, 23))
        self.valveFilter.setStyleSheet("color: rgb(255, 255, 255)")
        self.valveFilter.setObjectName("valveFilter")

        self.retranslateUi(filteredResults)
        QtCore.QMetaObject.connectSlotsByName(filteredResults)

    def retranslateUi(self, filteredResults):
        _translate = QtCore.QCoreApplication.translate
        filteredResults.setWindowTitle(_translate("filteredResults", "Filter Search"))
        self.labelFiltered.setText(_translate("filteredResults", "Filtered Results for: "))
        self.labelFiltered_2.setText(_translate("filteredResults", "[Insert filter button]"))
        self.psoitive.setText(_translate("filteredResults", "Top Positive"))
        self.specials.setText(_translate("filteredResults", "Specials"))
        self.negative.setText(_translate("filteredResults", "Top Negative"))
        self.valveFilter.setText(_translate("filteredResults", "Games by Valve"))
